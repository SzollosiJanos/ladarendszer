...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_fegyverujratolt2", cmd_kesztyuujratolt);
...
public Action cmd_kesztyuujratolt(int client, int args)
{
	if (IsValidClient(client))
	{
		GetPlayerData(client);
		CreateTimer(0.2, ASD, client);
	}
	return Plugin_Handled;
}

public Action ASD(Handle timer, int client){
	
	if(g_iGroup[client][GetClientTeam(client)]>0){
