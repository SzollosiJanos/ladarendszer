...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_fegyverujratolt", cmd_fegyverujratolt);
...
public Action cmd_fegyverujratolt(int client, int args)
{
	if (IsValidClient(client))
	{
		...
		int index=StringToInt(amount,10);
		if(index<=52 || index==99){
			char info[100];
			Format(info,sizeof(info),"%d|%d",client,index);
			new Handle:hPackedSQL = CreateDataPack();
			WritePackString(hPackedSQL, info);
			CreateTimer(0.2, ASD, hPackedSQL);
		}
	}
	return Plugin_Handled;
}

public Action ASD(Handle timer, Handle hPackedSQL){
	...
	if(index>=33 && index<=52 && index!=47){
		RefreshWeapon(client,index,true);
	}else{
